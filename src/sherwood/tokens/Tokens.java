...
public class Tokens {
    ...
    private int mode = 0;
	  ...
	  private BigFraction count = BigFraction.ZERO;
//   	private BigFraction multiplier = BigFraction.ONE; 
		private Stack<BigFraction> tuplets = new Stack<>();
    ...
    private void tokenize() {
        ...
        if (mode == 1) {
            String s = "";
            int str = 0;
            while(program.charAt(pointer) != '#' || str == 1) {
              if (program.charAt(pointer) == '"') str = 1 - str;
              if (program.charAt(pointer) == '`') pointer++; 
       
              s += program.charAt(pointer++);
            }
            pointer++;
            tokens.add(new Bracing(s));
            mode = 1 - mode;
            tokenize();
            return;
          }
        if (program.charAt(pointer) == ' ')
            ...
            while (program.charAt(pointer) == 'z') {
              exprs.add(new Expression("^\"z\""));
              pointer++;
            ...
            while (program.charAt(pointer) == ' ') pointer++;
            BigFraction curr = new BigFraction(new BigInteger(b + ""), new BigInteger(a + ""));
            tuplets.push(curr);
//						multiplier = multiplier.multiply(curr);
            if (program.charAt(pointer) != '{') {
            ...
            tokens.add(new TimeSignature(a, b));
						...
						if (program.charAt(pointer) == '}') {
//							multiplier = multiplier.multiply(BigFraction.ONE.divide(tuplets.pop()));
						}
